	...
	App.view = {
		...
		initPageSlider: function (){
			...
			function route(event) {
				var pageToken, url, dir
					hash = window.location.hash;
				if ( hash === '#agenda' ) {
					pageToken = 'agenda';
					dir = 'right';
				} else if ( hash === '#speakers' ) {
					pageToken = 'speakers';
					dir = 'right';
		...
		initPage: function( $page ){
			...
			//home
			if ( pageID == 'pgHome' ) {
				//trigger nav animating in
				setTimeout(function(){ $page.find('.nav').addClass('active'); }, 100);
			} 
			//agenda
			else if ( pageID == 'pgAgenda' ) {
				//init flickable for agneda dates 
