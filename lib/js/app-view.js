	...
	App.view = {
			...
			var vpW = $(document).width(),
				...
				opts = $.extend(
				{
					...
					onScroll: function(e, seg){
						...
						if ($.os.android) { setTimeout( document.body.scrollTop = 0, 900)  } // change height of tabs only if android  
		...
		initPage: function( $page ){
			...
			//agenda
				...
				// change height of tabs only if android 
				function setHeight(){
				 var    $flickable = $('.flickableContainer'), 
						$dayTabM = $flickable.find('#m'),
						$dayTabT = $flickable.find('#t'),
						$dayTabW = $flickable.find('#w'),
						$newHeightM = $dayTabM.find('.scroller').height(),
						$newHeightT = $dayTabT.find('.scroller').height(),
						$newHeightW = $dayTabW.find('.scroller').height(),
						numbers_array = [$newHeightM, $newHeightT, $newHeightW],   		        // put tab heights into an array
						biggestHeight = Math.max.apply( null, numbers_array );   		        // compare and get the largest height 
						
						console.log('hi'+ biggestHeight); 
						$('.pageContent').css('height',biggestHeight);  	    				// apply the largest height to all tabs  
				}
				
				if ($.os.android) { setHeight(); }  											// change height of tabs only if android 
